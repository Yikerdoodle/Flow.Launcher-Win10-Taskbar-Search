...
| **Start menu** (setting `ShowStartMenuPanel`, Left Bottom position): with an empty query, Flow shows a copy of the Windows 10 Start menu left of the home results, without the query box and preview; typing switches to the normal search layout, and clearing the query switches back. The rail (☰ with hover-to-expand, Pinned tiles, All apps, account with Change account settings / Lock / Sign out, Settings, Power with Sleep / Shut down / Restart) and the All apps list are measured from the real Start menu to within a pixel, including its text sizes under the Windows text size setting. All apps comes from the Windows Apps folder, the same source Start uses, with Start's folders, "Recently added" (Expand / Collapse), letter headers that open the letter grid, Start's own folder icon and its hover light. Windows keeps its "first seen" dates private, so Flow keeps its own record for Recently added. Pinned tiles is still to come. | `StartMenu/`, `MainWindow.xaml(.cs)`, `Resources/Dark.xaml`, `Resources/Light.xaml` |
