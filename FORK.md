...
| **Windows search icon**: Flow's icon (taskbar pin, tray, windows, notifications) is the Windows 10 taskbar search magnifier, drawn as vectors fitted to the real one's pixels at 150% scaling. `Scripts/search-icon.ps1` makes the icons. On first start Flow pins its own taskbar button with the tooltip "Type here to search" (Flow's translation of the search box text), replacing a plain Flow Launcher pin: a shortcut of that name in `%LOCALAPPDATA%\FlowLauncher` with its own AppUserModelID, since the taskbar names a pin after the app its ID resolves to. It does this once (setting `TaskbarSearchButtonSetUp`), so unpinning it later sticks. | `Resources/app.ico`, `Images/app.ico`, `Images/app.png`, `Scripts/search-icon.ps1`, `Helper/TaskbarSearchButton.cs` |
